...
const CREW = [
  { name:'Reid Wiseman',   role:'Commander',          agency:'NASA', flag:'🇺🇸',
    ...
    prev:'ISS Expedition 41 · 2014',        color:'var(--accent-hi)', initials:'RW' },
  { name:'Victor Glover',  role:'Pilot',              agency:'NASA', flag:'🇺🇸',
    ...
    prev:'SpX-Crew 1 · 2020–21',            color:'var(--green)',     initials:'VG' },
  { name:'Christina Koch', role:'Mission Specialist', agency:'NASA', flag:'🇺🇸',
    ...
    prev:'ISS Expedition 59–62 · 2019–20',  color:'#c084fc',          initials:'CK' },
  { name:'Jeremy Hansen',  role:'Mission Specialist', agency:'CSA',  flag:'🇨🇦',
    ...
    prev:'First spaceflight',               color:'var(--amber)',     initials:'JH' },
];
            ...
            {CREW.map((m, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background:   isActive ? 'rgba(45,125,210,0.09)' : 'var(--glass)',
                    border:       `1px solid ${isActive ? 'rgba(91,163,245,0.35)' : 'var(--border)'}`,
                    borderRadius: 8,
                    padding:      16,
                    cursor:       'pointer',
                ...
                >
                  {/* Avatar row */}
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom: isActive ? 12 : 6 }}>
                    <div
                      className="crew-avatar"
                      style={{
                        background: 'rgba(91,163,245,0.1)',
                        border:     '1.5px solid rgba(91,163,245,0.3)',
                        color:      m.color,
                        fontSize:   12,
                      }}
                    >
                    ...
                    <div>
                      <div className="f-display" style={{ fontSize:12, fontWeight:700, color:'#fff', letterSpacing:'0.03em' }}>
                        {m.name}
                      </div>
                      <div className="eyebrow" style={{ color:m.color, marginTop:2 }}>{m.role}</div>
                      <div className="eyebrow" style={{ fontSize:8, marginTop:1 }}>{m.flag} {m.agency}</div>
                    </div>
                  </div>
